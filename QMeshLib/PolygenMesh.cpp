...
void PolygenMesh::_buildDrawShadeList(bool bVertexNormalShading)
{
    ...
    for(Pos=meshList.GetHeadPosition();Pos!=NULL;meshIndex++) {
        ...
        for(PosFace=(mesh->GetFaceList()).GetHeadPosition();PosFace!=NULL;)
        {
			...
			//draw topology optimization result
			if (mesh->runTopOpt == true) {
				if (face->inner == true) {
					if (face->GetRightTetra()->dense > 0.5 && face->GetLeftTetra()->dense > 0.5) continue;
					else if (face->GetRightTetra()->dense < 0.5 && face->GetLeftTetra()->dense < 0.5) continue;
				}
				else {
					if (face->GetLeftTetra() == NULL && face->GetRightTetra()->dense < 0.5) continue;
					else if (face->GetRightTetra() == NULL && face->GetLeftTetra()->dense < 0.5) continue;
				}
				glColor3f(0.6f, 0.6f, 0.6f);
				if (mesh->drawDeformationField == true)
				{
					QMeshTetra* Tetra;
					if (face->GetLeftTetra() == NULL) Tetra = face->GetRightTetra();
					else Tetra = face->GetLeftTetra();
					_changeValueToColor(mesh->deformDrawValue[0], mesh->deformDrawValue[1], Tetra->ARAPenergy, rr, gg, bb);
					//_changeValueToColor(mesh->deformDrawValue[0], mesh->deformDrawValue[1], Tetra->rVol, rr, gg, bb);
					//_changeValueToColor(1.05, 0.95, Tetra->rVol, rr, gg, bb);
					glColor3f(rr, gg, bb);
				}
			}
			//These are the normal case
				...
				if (face->m_nIdentifiedPatchIndex < 0) {
					...
					/*else if (face->isFixedDraw) {
						glColor3f(0.3, 0.8, 1.0);
					}*/
					else if (face->isHandleDraw) {
...
void PolygenMesh::_buildDrawNodeList()
{
    ...
    for(Pos=meshList.GetHeadPosition();Pos!=NULL;meshIndex++) {
        ...
        for(PosNode=(mesh->GetNodeList()).GetHeadPosition();PosNode!=NULL;) {
            ...
            /* Added by ZTY 2020-03-01 */
            // for collision detection.
            else if (node->isCollision) {
                rr = 1; gg = 0; bb = 0;
            }
            else if (node->iscollided) {
                rr = 0; gg = 0; bb = 0;
            }
            else if (node->isSupportNode) {
                rr = 0.9; gg = 0.9; bb = 0.9;
            }
            // END
...
void PolygenMesh::_buildDrawNodeNormalList()
{
    ...
    for(GLKPOSITION meshPos=meshList.GetHeadPosition();meshPos!=NULL;) {
        ...
        for(GLKPOSITION Pos=mesh->GetNodeList().GetHeadPosition();Pos!=NULL;){
            ...
            double n[3];
            /* Added by ZTY 2020-03-01 */
            // for Node normal display.
            if (node->isGcodeNode == true) { 
                n[0] = node->m_printNormal[0];
                n[1] = node->m_printNormal[1];
                n[2] = node->m_printNormal[2];
            }
            else {
                node->CalNormal(n);
            }
